...
print("Sum of Sales after Grouping by Company Column : \n",bycomp.sum(numeric_only=True))  #person column cannot be used for mean as it contains strings
print("Standard Deviation of Sales after Grouping by Company Column : \n",bycomp.std(numeric_only=True))  #this used to be the default but has changed now
#person column cannot be used for mean as it contains strings

print("Sum of Sales after Grouping by Company Column For FB : \n",bycomp.sum(numeric_only=True).loc['FB'])  #person column cannot be used for mean as it contains strings

#repeating above usage of groupby without using an extra variable
print("Sum of Sales after Grouping by Company Column For FB : \n",df.groupby('Company').sum(numeric_only=True).loc['FB'])  #person column cannot be used for mean as it contains strings

print("Groupby Count function on Company Column : \n",df.groupby('Company').count())

print("Max function on Company Column : \n",df.groupby('Company').max()) #strings are checked alphabetically
...
print("Groupby along with description along with company selection : \n",df.groupby('Company').describe().loc['FB']) #.loc is required if transpose is not used
